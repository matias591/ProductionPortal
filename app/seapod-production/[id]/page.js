...
export default function SeapodBuildDetails({ params }) {
  ...
  // Safe Params Unwrap
  useEffect(() => {
    Promise.resolve(params).then((res) => setSeapodId(res.id));
  }, [params]);

  useEffect(() => {
    if (seapodId) fetchData();
  }, [seapodId]);
      ...
      <main className="flex-1 ml-64">
        ...
        <div className="p-8 max-w-5xl mx-auto">
            <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
                ...
                <table className="w-full text-left">
                    <thead className="text-xs font-bold text-slate-400 uppercase border-b border-slate-200">
                        <tr>
                            ...
                            <th className="px-6 py-3 w-48">Serial Number</th>
                        ...
                        {items.map(item => (
                            <tr key={item.id} className="group hover:bg-slate-50">
                                ...
                                <td className="px-6 py-3">
                                    <input 
                                        className="w-full border border-slate-200 rounded px-2 py-1.5 text-sm focus:border-[#0176D3] outline-none text-[#0176D3] font-medium placeholder-slate-300"
                ...
                <button onClick={addItem} className="w-full py-4 text-sm font-bold text-slate-500 hover:text-[#0176D3] border-t border-slate-200 flex items-center justify-center gap-2 transition-colors">
                    <Plus size={16}/> Add Extra Component
                </button>
            </div>
        </div>
