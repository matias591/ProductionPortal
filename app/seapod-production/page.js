...
import { Plus, Search, Cpu, ChevronRight, Download, Trash2, Box, User } from 'lucide-react';
...
export default function SeapodList() {
  ...
  useEffect(() => { checkUser(); fetchSeapods(); fetchTemplates(); }, []);
  ...
  function exportList() {
    const dataToExport = seapods.map(s => ({
        "Serial": s.serial_number, "Template": s.template_name, "Status": s.status, 
        "HW Ver": s.hw_version, "SW Ver": s.sw_version, "Seapod Ver": s.seapod_version, 
        "Order #": s.order_number || '-', "Created By": s.created_by || '-'
    }));
    const ws = XLSX.utils.json_to_sheet(dataToExport);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Seapod List");
    XLSX.writeFile(wb, "Seapod_Production_List.xlsx");
  }
        ...
        <div className="bg-white border border-slate-200 rounded-lg shadow-sm overflow-hidden">
            <table className="w-full text-left">
                <thead className="bg-slate-50 border-b border-slate-200 text-xs font-bold text-slate-500 uppercase">
                    <tr><th className="px-6 py-4">Seapod Serial</th><th className="px-6 py-4">Template</th><th className="px-6 py-4">Versions</th><th className="px-6 py-4">Status</th><th className="px-6 py-4 text-right">Action</th></tr>
                </thead>
