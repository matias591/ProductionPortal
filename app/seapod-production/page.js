...
export default function SeapodList() {
  ...
  async function fetchSeapods() {
    // --- UPDATED SORTING: Created At Descending ---
    const { data } = await supabase.from('seapod_production').select('*').order('created_at', { ascending: false });
    setSeapods(data || []);
  }
  
  async function fetchTemplates() {
    // --- UPDATED SORTING: Respect Drag & Drop Order ---
    const { data } = await supabase.from('seapod_templates').select('*').order('sort_order', { ascending: true });
    setTemplates(data || []);
  }
  ...
  function exportList() {
    const dataToExport = seapods.map(s => ({
        ...
        "Order #": s.order_number || '-', "Created By": s.created_by || '-', "Created Date": new Date(s.created_at).toLocaleDateString()
    }));
            ...
            <table className="w-full text-left">
                <thead className="bg-slate-50 border-b border-slate-200 text-xs font-bold text-slate-500 uppercase">
                    <tr>
                        ...
                        <th className="px-6 py-4">Created</th> {/* NEW HEADER */}
