              ...
              <div className="flex flex-col md:flex-row justify-between items-start gap-4">
                ...
                <div className="flex items-end gap-3">
                    ...
                    <div className="flex flex-col items-end">
                        ...
                        <option value="New">New</option><option value="In preparation">In preparation</option><option value="In Box">In Box</option><option value="Ready for Pickup">Ready for Pickup</option>{(isAdmin || order.status === 'Shipped') && <option value="Shipped">Shipped</option>}
                        </select>
                    </div>
                </div>
              </div>
          ...
          <main className="max-w-[1600px] mx-auto px-6 py-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-1 space-y-6">
                ...
                <div className="bg-white border border-slate-200 rounded-lg shadow-sm overflow-hidden">
                    <div className="px-5 py-3 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center"><h3 className="text-xs font-bold text-slate-500 uppercase tracking-wide flex items-center gap-2"><Paperclip size={14}/> Attachments ({files.length})</h3>{isAdmin && !isLocked && (<label className="cursor-pointer text-xs font-bold text-[#0176D3] hover:underline flex items-center gap-1">{uploading ? 'Uploading...' : '+ Upload'}<input type="file" className="hidden" onChange={handleFileUpload} disabled={uploading || isLocked} /></label>)}</div>
